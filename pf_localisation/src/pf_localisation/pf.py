...
class PFLocaliser(PFLocaliserBase):
    ...
    def particle_clustering(self, particlePoses):
        ...
        for pose in particlePoses: #Takes each individual particle in the particle poses array
            ...
            if  variableNew == self.CLOUD_POINTS*self.TOP_WEIGHT_PERCENTAGE:
                break
        ...
        if standardDeviation > 0.2: #If standard deviation its greater than this, don't consider the particle
            remainingParticlesPoses = [] #creates an array for the new particles
            if  standardDeviation >= 1:
                self.CLOUD_POINTS = 400
                self.TOP_WEIGHT_PERCENTAGE = 0.97
                #print("New Cloud points", self.CLOUD_POINTS)
                self.initialise_particle_cloud(self.estimatedpose)
            if 0.5 <= standardDeviation < 1.0:
                self.CLOUD_POINTS = 300
                self.TOP_WEIGHT_PERCENTAGE = 0.96
                #print("New Cloud points", self.CLOUD_POINTS)
                self.initialise_particle_cloud(self.estimatedpose)
            if 0.3 <= standardDeviation < 0.5:
                self.CLOUD_POINTS = 200
                self.TOP_WEIGHT_PERCENTAGE = 0.94
                #print("New Cloud points", self.CLOUD_POINTS)
                self.initialise_particle_cloud(self.estimatedpose)
            if standardDeviation < 0.3:
                self.CLOUD_POINTS = 150
                self.TOP_WEIGHT_PERCENTAGE = 0.92
                #print("New Cloud points", self.CLOUD_POINTS)
                self.initialise_particle_cloud(self.estimatedpose)
    ...
    def initialise_particle_cloud(self, initialpose):
        ...
        print("CurrentCloudPoints",self.CLOUD_POINTS)
    ...
    def update_particle_cloud(self, scan):
        ...
        height = self.occupancy_map.info.height #The height of the map so particle only span inside the heigh of the map
        resolution = self.occupancy_map.info.resolution #gives the resolution of the map
        remainingCloudPoints = cloudPoints - len(heaviestParticles) # Is the number of cloud points we are now randomly determining
